        ...
        <div className={`fixed inset-0 ${isDarkMode ? 'bg-[#0a0a12]' : 'bg-white'}`}>
            ...
            <div className="relative z-20 h-full w-full flex flex-col safe-area-top overflow-y-auto no-scrollbar pb-safe pb-4">
                ...
                <main className="flex-1 px-5 relative z-10">
                    <h2 className={`text-[16px] font-bold font-satoshi mb-[14px] uppercase ${isDarkMode ? 'text-white' : 'text-black'}`}>{category.title}</h2>
                                ...
                                <div key={faq.id} className="flex flex-col">
                                    ...
                                    {isExpanded && (
                                        <div className="pb-4 animate-in fade-in slide-in-from-top-1 duration-200">
                                            <div className={`w-[calc(100%+20px)] mx-[-10px] h-[1px] border-t border-dashed mb-4 ${isDarkMode ? 'border-white/20' : 'border-[#E6E8EB]'}`} />
                                            <p className={`text-[12px] leading-relaxed font-satoshi font-light ${isDarkMode ? 'text-white' : 'text-black'}`}>
                                                {faq.answer}
                                            </p>
                                        </div>
                                    )}

                                    {idx < category.faqs.length - 1 && <div className={`w-[calc(100%+20px)] mx-[-10px] h-[1px] ${isDarkMode ? 'bg-white/5' : 'bg-[#E9EAEB]'}`} />}
                                </div>
                            );
                        })}
                    </div>
                    ...
                    <div className="mt-32 pb-4">
                        <div className="flex flex-col items-start">
                            ...
                            <p className={`font-medium text-[14px] font-satoshi mt-[2px] px-[2px] ${isDarkMode ? 'text-white/40' : 'text-black/40'}`}>
