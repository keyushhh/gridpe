...
import {
  tiers,
  tierSettingsCardMap,
  tierSettingsCardMapLight,
  tierCarouselActiveMap,
  tierCarouselInactiveMap,
  tierCarouselActiveMapDark,
  tierCarouselInactiveMapDark,
  tierChipColorMap,
} from '@/lib/walletTiers';
...
const WalletSettings = () => {
  ...
  const handleUpgrade = () => {
    ...
    if (nextTier) {
      navigate(ROUTES.SUBSCRIPTION_DETAILS, {
  ...
  return (
    ...
    >
      ...
      <div className="shrink-0 relative flex items-center justify-between w-full px-5 pt-4 pb-0 z-10">
        ...
        <h1
          className={`text-[22px] font-medium tracking-normal text-center absolute left-1/2 -translate-x-1/2 ${isDarkMode ? 'text-white' : 'text-black'}`}
        >
          Wallet Settings
        </h1>
      ...
      <div className="flex-1 w-full overflow-y-auto no-scrollbar pb-[40px]">
        ...
        <div className="px-5 mb-[20px] mt-[28px]">
          <div
            className={`relative w-full rounded-[28px] overflow-hidden ${!isDarkMode ? 'border border-border' : ''}`}
            style={{
            ...
            <div className="relative z-10 flex flex-col h-full">
              ...
              <div className="mt-[17px] mb-[17px] px-6">
                {walletTier === 'Supreme' ? (
                  <button
                    disabled
                    className="mx-auto min-h-[48px] flex items-center justify-center rounded-full text-white text-[16px] font-medium transition-transform opacity-50 cursor-not-allowed bg-primary"
                    style={{ width: '326px', maxWidth: '100%', padding: '8px' }}
                  >
                    Current Plan
                  </button>
                ) : (
                  <button
                    ...
                    className={`mx-auto min-h-[48px] flex items-center justify-center rounded-full text-white text-[18px] font-medium active:scale-95 transition-transform bg-primary ${scheduledDowngrade ? 'opacity-50 cursor-not-allowed' : ''}`}
                    style={{ width: '326px', maxWidth: '100%', padding: '8px' }}
        ...
        <div className="mb-2">
          <div className="flex overflow-x-auto no-scrollbar px-5 gap-4 pb-4 snap-x snap-mandatory">
            {tiers.map(tier => {
              ...
              if (isDarkMode) {
                cardBg = isActive
                  ? tierCarouselActiveMapDark[tier.name]
                  : tierCarouselInactiveMapDark[tier.name];
              } else {
                cardBg = isActive
                  ? tierCarouselActiveMap[tier.name]
                  : tierCarouselInactiveMap[tier.name];
              }
              ...
              return (
                <div
                  key={tier.name}
                  onClick={() =>
                    navigate(ROUTES.WALLET_TIER.replace(':tierId', tier.name.toLowerCase()))
                  }
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    ...
                    >
                      <div>
                        <p
                          className={`text-[12px] font-medium mb-0 ${isDarkMode ? 'text-white/50' : 'text-black/40'}`}
                        >
                          Verification
                        </p>
                        <p
                          className={`text-[12px] font-medium whitespace-nowrap overflow-hidden text-ellipsis ${isDarkMode ? 'text-white' : 'text-black'}`}
                        >
                          {tier.verification}
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <p
                          className={`text-[12px] font-medium leading-snug ${isDarkMode ? 'text-white' : 'text-black'}`}
                        >
                          {tier.limitations}
                        </p>
                      </div>
        ...
        <div className="px-5 pb-5 flex flex-col gap-4">
          <div>
            <h3
              className={`text-[16px] font-medium mb-[6px] ${isDarkMode ? 'text-white' : 'text-black'}`}
            >
              How to Upgrade?
            </h3>
            <ul className="flex flex-col gap-[2px] list-disc pl-6">
              <li
                className={`text-[16px] font-light leading-[1.4] tracking-[-0.3px] ${isDarkMode ? 'text-white' : 'text-black'}`}
              >
          ...
          <div>
            ...
            <p
              className={`text-[16px] font-light leading-[1.4] tracking-[-0.3px] mb-4 ${isDarkMode ? 'text-white' : 'text-black'}`}
            >
              Wallets in India are governed by RBI-regulated limits to ensure fund security and
              prevent misuse. Your tier helps us serve you better, safely, and responsibly.
            </p>
          </div>
          ...
          <div className="mt-4">
            <button
              ...
              className={`w-full min-h-[48px] flex items-center justify-center rounded-full text-[14px] font-medium border active:scale-95 transition-transform ${
                isDarkMode
            ...
            <p
              className={`text-[10px] text-center mt-2 ${isDarkMode ? 'text-muted-foreground/50' : 'text-muted-foreground/50'}`}
            >
