...
const WalletTransactionHistory = () => {
  ...
  const isAnyFilterActive =
    filters.date !== 'All Time' || filters.type !== 'All' || filters.method !== 'All';
  const isNewUser = () => {
  ...
  const filteredTransactions = useMemo(() => {
    return walletTransactions.filter(tx => {
      ...
      // 3. Type Filter
      if (filters.type !== 'All') {
        if (filters.type === 'Cash Order' && !tx.description.toLowerCase().includes('cash order'))
          return false;
        if (filters.type === 'Withdrawal' && !tx.description.toLowerCase().includes('withdrawal'))
          return false;
        if (
          filters.type === 'Wallet Top-Up' &&
          !(
      ...
      if (filters.method !== 'All') {
        ...
        if (
          ...
          !(
            tx.description.toLowerCase().includes('netbanking') ||
            tx.description.toLowerCase().includes('withdrawal')
          )
        )
          return false;
        if (filters.method === 'Cards' && !tx.description.toLowerCase().includes('card'))
          return false;
      }
      return true;
    });
      ...
      <div className="relative" ref={triggerRef}>
        <div
          ...
          style={{
            ...
            borderRadius: '8px',
            border: isActive
              ? '0.63px solid hsl(var(--primary))'
              : '0.63px solid hsl(var(--border) / 0.12)',
          }}
        >
              ...
              <div className="flex flex-col gap-[12px]">
                {items.map((item, index) => (
                  <span
                    ...
                    className={`text-[12px] font-medium font-sans leading-[120%] cursor-pointer transition-colors ${selectedValue === item ? 'text-primary' : 'text-white hover:text-white/80'}`}
                  >
                    {item}
                  </span>
                ))}
  ...
  const getTransactionDisplay = (tx: WalletTransaction) => {
    ...
    if (tx.metadata?.isFx) {
      ...
      desc.includes('phone pe') ||
      desc.includes('upi id') ||
      desc.includes('upi')
    ) {
    ...
    return (
      ...
      >
        ...
        <div
          ...
          style={{
            backgroundImage: `url(${ASSETS.TRANSACTION_DETAILS_LIGHT})`,
        ...
        >
          ...
          {/* Transaction ID Pill */}
          <div
            className="mt-[19px] w-[213px] h-[40px] flex items-center justify-center bg-muted rounded-full border border-border/5"
            onClick={handleCopy}
          >
            <span
            ...
            {(() => {
              ...
              const rowData = [
                { label: 'Transaction Type', value: title },
                {
                  ...
                  value: subtitle === 'Wallet Top Up' ? 'Wallet Top Up' : subtitle,
                },
                {
                  ...(tx.metadata?.isFx
                    ? [
                        {
                          ...
                          value: `${currencySymbols[tx.metadata.toCurrency as string] || ''}${Number(tx.metadata.receiveAmount || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
                        },
                        {
                          label: 'Exchange Rate',
                          value: `1 ${tx.metadata.fromCurrency} = ${currencySymbols[tx.metadata.toCurrency as string] || ''}${Number(tx.metadata.fxRate || 0).toFixed(2)}`,
                        },
                      ]
                    : []),
                },
          ...
          >
            ...
            <span className="underline text-primary cursor-pointer">Click here.</span>
          </p>
        </div>
        {/* Close Button */}
        <button
  ...
  return (
    ...
    >
      ...
      <div className="w-full px-[19px] mt-[28px] z-10">
        <div
          className="w-full flex items-center px-[16px] rounded-full bg-muted/30 dark:bg-muted/10 border border-border"
          style={{
            height: '44px',
          }}
          onClick={e => e.stopPropagation()}
        >
          ...
          <img
            ...
            style={{
              filter: isDarkMode ? 'brightness(0) invert(1)' : 'grayscale(1) brightness(0)',
            }}
          />
          {/* Input */}
          <input
            ...
            className="ml-[16px] w-full bg-transparent border-none outline-none text-foreground placeholder:text-muted-foreground text-[14px] font-normal leading-[140%]"
          />
        </div>
      </div>
      {/* Filter Row */}
        ...
        {(() => {
          ...
          return sortedDates.map((dateKey, index) => {
            ...
            const dateObj = new Date(
              Number(dateKeyParts[2]),
              Number(dateKeyParts[1]) - 1,
              Number(dateKeyParts[0])
            );
            const heading = formatDate(dateObj, {
              type: 'short',
              showTodayYesterday: true,
            }).toUpperCase();
            return (
              <div key={dateKey} className={index === 0 ? '' : 'mt-[24px]'}>
                <h2 className="text-muted-foreground text-[13px] font-medium leading-[120%] tracking-[1px] uppercase ml-[4px]">
                  {heading}
                </h2>
                <div
                  className="mt-[12px] flex flex-col bg-muted/30 dark:bg-muted/10 border border-border rounded-[16px] p-[14px_16px]"
                  style={{
                ...
                >
                  {transactions.map((tx, txIndex) => {
                    ...
                    const desc = tx.description?.toLowerCase() || '';
                    const isTopUp =
                      type === 'credit' || desc.includes('top-up') || desc.includes('top up');
                    // Deduce Icon
                    const isMoneyIn =
                      type === 'credit' ||
                      type === 'deposit' ||
                      ...
                      desc.includes('top up');
                    const isUPIWithdrawal =
                      desc.includes('withdrawal') &&
                      (desc.includes('upi') || subtitle.includes('UPI'));
                    const icon = isUPIWithdrawal
                      ? ASSETS.WALLET_DEBITED
                      : isMoneyIn
                        ? ASSETS.CREDITED_ARROW
                        : ASSETS.DEBITED_ARROW;
                    // Deduce Color
                    let amountClass =
                    ...
                    return (
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div className="flex items-start gap-[16px]">
                            ...
                            <div className="flex flex-col gap-[2px]">
                              ...
                              <span className="text-muted-foreground text-[13px] font-normal leading-[120%]">
                                {subtitle}
                              </span>
                            </div>
                          </div>
                          <div className="text-right flex flex-col items-end gap-[4px]">
                            <span
                              className={`text-[15px] font-bold leading-[120%] tracking-[-0.3px] ${amountClass}`}
                            >
                              {tx.type === 'credit' ||
                            ...
                            <span className="text-muted-foreground text-[12px] font-normal leading-[120%]">
                        ...
                        {txIndex < transactions.length - 1 && (
                          <div
                            className="h-[1px] bg-border mt-[8px] mb-[12px]"
