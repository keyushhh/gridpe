...
const SuccessScreen = () => {
  ...
  useEffect(() => {
    if (kycStatus === 'verified') {
      ...
      const timer = setTimeout(() => {
        ...
        navigate(isFxFlow ? ROUTES.FX_EXCHANGE : ROUTES.HOME, { replace: true });
      }, 2500);
  ...
  const getHeading = () => {
    ...
    if ((kycStatus === 'pending' || kycStatus === 'in_review') && isWaitingForRealtime)
      return 'Finalizing verification with Didit...';
    return 'Your KYC details has been submitted successfully!';
  };
  const getBodyText = () => {
    if (kycStatus === 'verified')
      return "You're all set! Your account is now verified and all features are unlocked.";
    if ((kycStatus === 'pending' || kycStatus === 'in_review') && isWaitingForRealtime)
      return "We're just wrapping things up. This usually takes a few seconds.";
  ...
  return (
    ...
    >
      ...
      <div className="w-full pt-4 flex justify-center relative z-10">
        <h1
          className={`${isDarkMode ? 'text-white font-satoshi' : 'text-black font-sans'} text-[22px] font-medium`}
        >
          KYC
        </h1>
      </div>
      ...
      <div className="pb-4 space-y-4 relative z-10 w-full flex flex-col items-center px-5">
        ...
        >
          {kycStatus === 'verified'
            ...
            : isRefreshing
              ? 'Checking database...'
              : 'Waiting... Tap to Refresh'}
        </button>
        {/* Disclaimer — 12px below CTA */}
        <p
          className={`${isDarkMode ? 'text-white/40' : 'text-black/40'} text-[12px] text-center font-sans mt-[12px]`}
        >
          {kycStatus === 'verified'
      ...
      <div className="mt-auto safe-bottom pb-4 px-4">
        <p
          className={`${isDarkMode ? 'text-white' : 'text-black/60'} text-[13px] text-center leading-snug font-sans`}
        >
          {kycStatus === 'verified'
