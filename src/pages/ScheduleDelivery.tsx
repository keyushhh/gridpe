...
const ScheduleDelivery = () => {
  ...
  const generateTimeSlots = (currentMeridiem: string, extraTime?: string) => {
    ...
    if (
      extraTime &&
      extraTime.toLowerCase().includes(currentMeridiem.toLowerCase()) &&
      !slots.includes(extraTime)
    ) {
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-5 no-scrollbar pb-[280px] relative z-10">
        ...
        <div className="mb-6">
          ...
          <p
            className={`text-[14px] font-normal leading-tight ${isDarkMode ? 'text-white/60' : 'text-black'}`}
          >
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-4">
            <div className="flex items-center gap-2">
              ...
              />
              <span className={`text-[15px] font-bold ${isDarkMode ? 'text-white' : 'text-black'}`}>
                Time
              </span>
            </div>
            <span
              className={`text-[15px] font-bold transition-colors duration-300 ${isInvalidTime ? (isDarkMode ? 'text-white/30' : 'text-black/30') : isDarkMode ? 'text-white' : 'text-black'}`}
            >
              {selectedTime}
            </span>
          </div>
          ...
          >
            ...
            <div className="flex items-center justify-between pl-8 pr-16 h-full">
              ...
              <div
                ...
                style={{
                  backgroundImage: `url(${isDarkMode ? ASSETS.CLOCK_BASE : ASSETS.CLOCK_LIGHT})`,
                  backgroundSize: 'cover',
                }}
              ...
              >
                {/* Pivot */}
                <div
                  className={`absolute top-1/2 left-1/2 w-1.5 h-1.5 rounded-full -translate-x-1/2 -translate-y-1/2 z-20 shadow-sm ${isDarkMode ? 'bg-white' : 'bg-black opacity-100'}`}
                />
                {/* Hour Hand */}
                <div
        ...
        <div className="mt-6 text-left px-1">
          <p
            className={`text-[14px] font-medium font-sans leading-snug ${isDarkMode ? 'text-white' : 'text-black'}`}
          >
            Note: We'll do our best to deliver at your selected time. Actual timing may vary
            slightly based on rider availability.
          </p>
        </div>
      </div>
