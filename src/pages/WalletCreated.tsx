...
const WalletCreated = () => {
  ...
  const renderStatusIndicator = () => {
      ...
      return (
        <div className="mt-[16px]">
          <div className="flex items-center">
            <div
              className={`w-[14px] h-[14px] rounded-full ${showWarning ? 'bg-yellow-500 shadow-[0_0_0_5px_rgba(250,204,21,0.17)]' : 'bg-green-500 shadow-[0_0_0_5px_rgba(92,255,0,0.17)]'}`}
            />
            <span
              className={`ml-[13px] text-[14px] font-medium font-sans ${isDarkMode ? 'text-white' : 'text-black'}`}
            >
              {isDowngradePending
          ...
          <p
            className={`mt-[10px] text-[12px] font-normal font-sans leading-snug ${isDarkMode ? 'text-white/60' : 'text-black/80'}`}
          >
            {showWarning
    ...
    if (Number(walletBalance) > 0 && latestTx) {
      ...
      const isFxOrCash =
        txDescription.includes('fx exchange') ||
        txDescription.includes('cash delivery') ||
        txDescription.includes('cash order');
      const isOnHold = status === 'held' && isFxOrCash;
      if (isOnHold) {
        statusClass = 'bg-yellow-500';
        shadowClass = 'shadow-[0_0_0_5px_rgba(250,204,21,0.17)]';
        ...
        txDescription.includes('cancelled') ||
        txDescription.includes('failed') ||
        txDescription.includes('released')
      ) {
        statusClass = 'bg-green-500';
        shadowClass = 'shadow-[0_0_0_5px_rgba(92,255,0,0.17)]';
        ...
        shadowClass = 'shadow-[0_0_0_5px_rgba(59,130,246,0.17)]';
      ...
      return (
        <div className="mt-[16px]">
          <div className="flex items-center">
            <div className={`w-[14px] h-[14px] rounded-full ${statusClass} ${shadowClass}`} />
            <span
              className={`ml-[13px] text-[14px] font-medium font-sans ${isDarkMode ? 'text-white' : 'text-black'}`}
            >
              {title}
          ...
          <p
            className={`mt-[10px] text-[12px] font-normal font-sans leading-snug ${isDarkMode ? 'text-white' : 'text-black'}`}
          >
            {description}
          </p>
        </div>
    ...
    return (
      <div className="mt-[16px]">
        <p
          className={`text-[13px] font-medium font-sans leading-tight tracking-tight ${isDarkMode ? 'text-white/90' : 'text-black/90'}`}
        >
          Uh ho! Looks like a little empty here, let’s fix that?
          <br />
  ...
  return (
    <div
      key={userId || 'wallet'}
      className="h-full w-full overflow-y-auto overscroll-auto flex flex-col safe-top relative bg-background"
      style={{
    ...
    >
      ...
      <div className="shrink-0 flex items-center justify-between w-full px-5 pt-4 pb-2 z-10">
        ...
        {/* Title */}
        <h1
          className={`text-[20px] font-medium font-sans ${isDarkMode ? 'text-white' : 'text-black'}`}
        >
          Wallet
        </h1>
        {/* Settings Button */}
      ...
      <div className="flex-1 w-full px-5 pt-4 overflow-y-auto no-scrollbar pb-[20px]">
        ...
        <div className="w-full relative shrink-0" style={{ width: '100%', minHeight: '200px' }}>
          ...
          <div className="relative w-full h-full px-5 pt-4 pb-[20px] flex flex-col">
            <div className="flex justify-between items-center">
              <span
                className={`${isDarkMode ? 'text-white' : 'text-black'} text-[15px] font-medium font-sans uppercase`}
              >
                Wallet Balance
              </span>
            </div>
            <div className="flex items-center justify-between mt-[12px]">
              <span
                className={`${isDarkMode ? 'text-white' : 'text-black'} text-[34px] font-bold font-sans`}
              >
                ₹{' '}
            ...
            <div className="mt-auto flex flex-col gap-1">
              <span
                className={`${isDarkMode ? 'text-white/80' : 'text-black/80'} text-[14px] font-medium font-sans`}
              >
                Wallet Limit:{' '}
                {dbLimit.toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}
              </span>
              {dbDailyLimit != null ? (
                <span
                  className={`${isDarkMode ? 'text-white/60' : 'text-black/60'} text-[12px] font-medium font-sans`}
                >
                  Daily Limit:{' '}
                  {dbDailyLimit.toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}
                </span>
              ) : dbTier === 'Supreme' ? (
                <span
                  className={`${isDarkMode ? 'text-white/60' : 'text-black/60'} text-[12px] font-medium font-sans`}
        ...
        {walletBalance === 0 && (
          <div
            ...
            style={{
              ...
              borderRadius: '13px',
              border: isDarkMode
                ? '1px solid hsla(var(--foreground) / 0.1)'
                : '1px solid hsla(var(--primary) / 0.1)',
            }}
          >
            <h3
              className={`text-[14px] font-medium font-sans mb-1 ${isDarkMode ? 'text-white' : 'text-black'}`}
            >
              Important:
            </h3>
            <p
              className={`text-[14px] font-normal font-sans leading-snug ${isDarkMode ? 'text-white' : 'text-black'}`}
            >
        ...
        <div className="mt-5 w-full mb-[20px]">
          <div className="flex justify-between items-center mb-[12px]">
            ...
            <button
              onClick={() => navigate(ROUTES.WALLET_TRANSACTION_HISTORY)}
              className="text-primary text-[14px] font-medium font-sans"
            >
              View All
            </button>
          </div>
          ...
          <div
            ...
            style={{
              borderTop: isDarkMode
                ? '2px solid hsla(var(--foreground) / 0.06)'
          ...
          {walletTransactions.length > 0 ? (
            <div className="w-full flex flex-col">
              ...
              <div className="flex justify-between items-center px-0 mb-[12px]">
                <div className="text-muted-foreground text-[12px] font-normal font-sans">
                  Details
                </div>
                <div className="text-right text-muted-foreground text-[12px] font-normal font-sans">
                  Price
                </div>
              </div>
              <div className="flex flex-col gap-[16px]">
                {walletTransactions.slice(0, 10).map(tx => {
                  ...
                  const icon = isMoneyIn ? ASSETS.WALLET_CREDITED : ASSETS.WALLET_DEBITED;
                    ...
                    <div key={tx.id} className="flex justify-between items-center gap-3">
                      <div className="flex items-center gap-[12px] flex-1 min-w-0">
                        ...
                        <div className="flex flex-col min-w-0 overflow-hidden">
                          ...
                          <span
                            className={`text-[11px] font-normal font-sans leading-none mb-[4px] truncate text-muted-foreground`}
                          >
                            {subtitle}
                          </span>
                          <span
                            className={`text-[11px] font-normal font-sans leading-none text-muted-foreground/60`}
                          >
                      ...
                      <div className="text-right shrink-0">
                        <span
                          className={`text-[13px] font-bold font-sans ${isDarkMode ? 'text-white' : 'text-black'}`}
                        >
            ...
            <div className="w-full py-10 flex items-center justify-center">
              <p className="text-muted-foreground text-[14px] font-normal font-sans text-center px-10">
                This screen’s more empty than
                <br />
